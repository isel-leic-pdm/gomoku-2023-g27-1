...
@Parcelize
data class Position private constructor(val lin: Int, val col: Int):Parcelable {
    ...
    companion object {
        ...
        operator fun invoke(l: Int, c: Int) : Position {
            require(l in 0 until BOARD_SIZE) { "Illegal coordinate line $l must be between 0 and $LAST_COORD" }
            require(c in 0 until BOARD_SIZE) { "Illegal coordinate col $c must be between 0 and $LAST_COORD" }
